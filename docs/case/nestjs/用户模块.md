...
```js
async update(params) {
  const { id, roleCodes, ...rest} = params
  let roles: RolesEntity[]

  try {
    if (roleCodes && Array.isArray(roleCodes) && roleCodes.length > 0) {
      roles = await this.rolesRepo
        .createQueryBuilder('roles')
        .where("roles.code IN (:...codes)", { codes: roleCodes })
        .getMany()
    }
  } catch(e) {
    throw new ApiException('查询失败', ApiErrorCode.FAIL, HttpStatus.OK)
  }
  ...
  try {
    ...
    if(user) {
      await this.usersRepo.save({
        ...user,
        roles,
        roleCodes: roles.map(v => v.code)
      })
    }
    return null
  } catch (e) {
    throw new ApiException(`更新失败, 发生异常: ${e}`, ApiErrorCode.FAIL, HttpStatus.OK)
  }
}
```
...
import { LoginParamsDto } from './dto/login_params.dto';
...
import { CreateUserRegisterDto } from './dto/creat_user_register.dto';
import { RedisCacheService } from './redisCache.service';
import * as nodemailer from 'nodemailer';
import { UniqueUser } from './dto/unique_user';
